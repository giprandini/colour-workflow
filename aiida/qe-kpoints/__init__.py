        ...
        if self._use_kpoints:
            try:
                ...
                force_kpoints_list = settings_dict.pop('FORCE_KPOINTS_LIST',
                                                       False)
                gamma_images = settings_dict.pop('GAMMA_IMAGES',
                                                       False)
                if force_kpoints_list and not gamma_images:
                    kpoints_list = kpoints.get_kpoints_mesh(print_list=True)
                    num_kpoints = len(kpoints_list)
                    has_mesh = False
                    weights = [1.] * num_kpoints
                elif force_kpoints_list and gamma_images:
                    kpoints_list = kpoints.get_kpoints_mesh_with_Gimages()
                    num_kpoints = len(kpoints_list)
                    has_mesh = False
                    weights = [1.] * num_kpoints
