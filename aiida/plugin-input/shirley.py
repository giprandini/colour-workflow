...
class ShirleyCalculation(JobCalculation):
    """
    Plugin for simple_ip.x (Shirley interpolation for IP optical properties)
    """

    def _init_internal_params(self):
        super(ShirleyCalculation, self)._init_internal_params()
        ...
        self._OUTPUT_DIELECTRIC_IM = 'aiida.eps_inter_im.dat'
        self._OUTPUT_DIELECTRIC_RE = 'aiida.eps_inter_re.dat'
        self._internal_retrieve_list = []
        self._default_namelists = ['INPUTSIMPLEIP']
        self._blocked_keywords = [
                                  ('INPUTSIMPLEIP','simpleip_in%outdir',self._OUTPUT_SUBFOLDER),
                                  ('INPUTSIMPLEIP','simpleip_in%prefix',self._PREFIX),
                                 ]
        self._parent_folder_type = (RemoteData)
        self._default_parser = 'shirley'
    ...
    def use_parent_calculation(self, calc):
        ...
        """
        if not isinstance(calc, (SimpleCalculation)):
            raise ValueError("Parent calculation must be a Simple "
                             "Calculation")
        try:
